...
const Councillors = ({ data }) => {
  const wards = data.contentfulWards.edges.sort((x, y) => {
    return x.node.name.toUpperCase() < y.node.name.toUpperCase() ? -1 : 1;
  });

  const councillors = data.contentfulCandidates.edges.sort((x, y) => {
    return x.node.name.toUpperCase() < y.node.name.toUpperCase() ? -1 : 1;
  });
  ...
  return (
    <Segment style={{ padding: "8em 0em" }} vertical>
      <Container text>
        ...
        <p>
          There are 15 Labour councillors representing you across Hornsey & Wood
          Green. Find out who is representing your local area.
        </p>
          ...
          wards.map(ward => (
            <Container key={ward.node.id + "-ward"}>
              ...
              {councillors.filter(councillor => {
                return councillor.node.ward.id === ward.node.id;
              }) ? (
                <Grid columns={3}>
                  <Grid.Row>
                    {councillors
                            ...
                            src={`
                                ...
                                councillor.node.image
                                  ? "https://res.cloudinary.com/codogo/image/fetch/h_530,w_430,c_fill,g_face,f_auto/https:" +
                                    councillor.node.image.file.url
                                  : profileImage
                              }`}
                            as={Link}
                            to={`/councillors/${slugify(councillor.node.name, {
                              lower: true
                            })}`}
                          />
