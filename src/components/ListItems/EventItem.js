...
import { Button, Divider, Grid, Header, Icon, Image } from "../toolbox";
  ...
  <Grid.Row key={event.node.id + "-newsitem"}>
    <Grid.Column>
      <EventThumbnail
        ...
        as={Link}
        to={"/events/" + slugify(event.node.title, { lower: true })}
      />
    </Grid.Column>

    <Grid.Column>
      <Link to={"/events/" + slugify(event.node.title, { lower: true })}>
        <Header as="h3">{event.node.title}</Header>
      </Link>

      <p style={{ color: "#aaaaaa" }}>
        {Moment(event.node.date).format("MMMM Do YYYY")}
      </p>
